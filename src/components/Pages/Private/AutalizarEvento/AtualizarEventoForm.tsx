...
const AtualizarEventoForm = () => {
    ...
    const detectChanges = useMemo((): { changes: TChangeItem[], riskScore: number } => {
        ...
        compareValues(currentFormData.acceptsPixPayment, originalData.acceptsPixPayment, "Aceitar pagamento por PIX", "high", 12)
        compareValues(currentFormData.acceptsCreditCardPayment, originalData.acceptsCreditCardPayment, "Aceitar pagamento por Cartão", "high", 12)
    ...
    useEffect(() => {
        if (eventData?.data && !isFormInitialized && eventCategories.length > 0) {
            ...
            const recurrenceData = event.Recurrence ? {
                ...
                day: event.Recurrence.day || undefined,
            ...
            const initialFormData = {
                ...
                acceptsPixPayment: (event as any).acceptsPixPayment !== false,
                acceptsCreditCardPayment: (event as any).acceptsCreditCardPayment !== false,
                isFree: !!event.isFree,
    ...
    const executeSubmit = async (data: TEventUpdate) => {
        try {
            ...
            const submitData: TEventUpdate = {
                ...
                acceptsPixPayment: data.acceptsPixPayment !== false,
                acceptsCreditCardPayment: data.acceptsCreditCardPayment !== false,
